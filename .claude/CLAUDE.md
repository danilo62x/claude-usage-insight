...
- The effective Claude config directory comes from `instance_id.effective_config_dir()` in every module, `api.py` included - never re-derived inline from `CLAUDE_CONFIG_DIR`. The helper resolves the path, so an inline copy addresses the credentials through a different spelling than the one every other module derives and than the one `config_dir_suffix()` hashes into the instance identity. `instance_id` imports only the standard library, which is what makes it importable from `api.py`; the reverse direction stays forbidden, because `__main__.py` needs it before `CLAUDE_CONFIG_DIR` is settled. `tests/test_api.py::test_matches_instance_id_helper` pins this with a spelling that only compares equal once resolved
- `_redact_home()` in `verbose.py` compares the home directory in both its literal and its resolved spelling, because the paths reaching it differ: the credentials path arrives resolved, while `sys.executable`, `sys._MEIPASS` and the `CLAUDE_CONFIG_DIR` row (which `__main__.py` stores resolved) keep the spelling they were given. Where the home directory is reached through a symlink or a junction those two differ, and matching only one of them prints the username in full - in the dump users attach to public issues. Resolving can fail there (a symlink loop raises `RuntimeError` up to Python 3.12, an unreachable network path an `OSError`), so it falls back to the literal spelling rather than not printing at all
...
- The EXE is built by the command and verified by reading `FileVersion` back out of the artifact - a leftover `dist/ClaudeUsageInsight.exe` from an earlier run looks identical otherwise. It is also smoke-tested, because the unit tests never load the frozen bundle, so a missing `datas` entry or hidden import surfaces nowhere else
- `build.py` **code signs** the EXE and refuses to finish if that fails, so an unsigned build can never reach a release page unnoticed. It signs against the certificate named in a git-ignored `signing.env` beside the script (thumbprint and timestamp server; no PIN is stored - the token asks for it on every run), and without that file it produces the unsigned EXE a contributor's clone builds. The decision around signtool is guarded by `tests/test_build.py` (no token needed - signtool is a recording stub there)
- The token is a physical device, so the build **blocks on a PIN dialog** partway through. Ask the user for it and give the command a generous timeout, or the wait for the PIN reads as a hung build. It is also why the release is cut here and not on a hosted runner
- This fork's `README.md` does not repeat the release checklist - `/releasing` is the single place that owns it, so the two can no longer drift apart

## Distribution
- This fork is **not** published to WinGet, and the upstream `winget.yml` / `winget-watch.yml` workflows were deleted with the fork. They submitted to the package id `jens-duttke.usage-monitor-for-claude` with a token this repository does not hold, so a release here would have failed the workflow at best and pushed a manifest at a package that is not ours at worst. Do not reinstate them without owning a package id
- The GitHub release is therefore the only distribution channel. Every release lists the SHA256 of the EXE, which is what lets a reporter check an antivirus warning against the published file
- The EXE this fork builds is **unsigned** unless a `signing.env` is configured locally. `README.md` must not claim a publisher name - upstream's text named Jens Duttke, and carrying that over would be false
...
- The SHA256 in the release notes carries a second job because of this: it is what turns "this is a false positive" from a promise into something the reporter can check. Keep it in every release
- Upstream code signs its EXE; this fork does not, unless a `signing.env` is configured locally. Never answer a report by pointing at a signature this build does not carry. The `!ml` verdicts weight the packaging anyway, which a signature does not change
- Options already weighed and rejected, so they do not get re-proposed: `--onedir` (it would help most, and it would break "single EXE, no installation" - the first feature bullet), and scripting the WDSI submission with a headless browser (it would put the Microsoft account credentials in CI secrets and fail silently when the form changes). What is left is a per-release submission to <https://www.microsoft.com/wdsi/filesubmission> as a "Software developer" dispute - the hash is fixed at build time, so it can be filed before publishing
